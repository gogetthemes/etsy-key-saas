...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      setError("Ошибка авторизации: " + decodeURIComponent(errorParam));
    }
  }, [searchParams]);
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    addActionLog("Начало входа", "pending", `Email: ${email}`);
    ...
    const res = await signIn("credentials", {
      ...
      if (res?.error) {
        addActionLog("Ошибка входа", "error", "Неверный email или пароль");
        setError("Неверный email или пароль");
      } else {
        addActionLog("Вход успешен", "success", `Пользователь ${email} вошел в систему`);
        window.location.href = "/dashboard";
      }
    } catch (err) {
      addActionLog("Ошибка входа", "error", "Ошибка сети");
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen p-4 bg-gray-50">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">Вход в систему</h1>
